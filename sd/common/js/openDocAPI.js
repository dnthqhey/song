...
/*담당자 이름 구하기*/
function getChgrNmpn(PRDNDT,PRDNNSTRGSTNO){
	var chgrNmpn = "";
	openAPI.getDetail(PRDNDT, PRDNNSTRGSTNO, function(result, error){
        if(error){
            openMsg('오류가 발생하였습니다.');
            return;
        }else{
        	//console.log(resultd);
        	//console.log(resultd.prsrvPdCd);
        	chgrNmpn = result.chgrNmpn;//resultd.chgrNmpn;
        	console.log(chgrNmpn);
        	return chgrNmpn;        
        }
      }                   
    );
...
/*보존기간 구하기*/
function getPrsrvPdCd(PRDNDT,PRDNNSTRGSTNO){
	var prsrvPdCd="";
	prsrvPdCd = openAPI.getDetail(PRDNDT, PRDNNSTRGSTNO, function(result, error){
        if(error){
            openMsg('오류가 발생하였습니다.');   
            return;
        }else{
        	//console.log(resultd);
        	//console.log(resultd.chgrNmpn);
        	var in_prsrvPdCd = result.prsrvPdCd;//resultd.prsrvPdCd;
        	console.log(prsrvPdCd);    
        	return in_prsrvPdCd;   
        }        
      }                   
    );
        ...
        if(error){
            ...
            if(result.length > 0){
                $.each(result, function(index, data){
                    ...
                    str += "<td class=\"ce vmid \">" + fulldate + "</td>";
                    str += "<td class=\"ce vmid end\">"+data.DEPTNM+"</td>";                    
                   // str += "<td class=\"ce vmid \">"+ getChgrNmpn(data.PRDNDT, data.PRDNNSTRGSTNO) +" </td>";                    
                    //str += "<td class=\"ce vmid end\"> "+ getPrsrvPdCd(data.PRDNDT, data.PRDNNSTRGSTNO) +" </td>";
                    str += "</tr>";
                    seq++;
                    no--;
                });

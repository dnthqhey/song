(function() {
    function _openAPI() {
        ...
        api.getList = function(callback) {
            ...
            $.ajax({
                type: 'GET',
                url: 'http://opengov.seoul.go.kr/openapi',
        ...
        api.getWonmunList = function(callback) {
        	...
        	var searchTitle = api.param.q;
        	var searchInfo = "?gvrncd=100019";
        	searchInfo += "&display=10";
            if(!api.param.pageNo)api.param.pageNo  = 1;
            ...
            for (var key in api.param) {
                ...
                if (pValue) {
                    switch (key) {
	                    case 'pageNo':
	                        pValue ? pr.page.offset = pValue : null;
	                    	searchInfo += "&pageno="+api.param.pageNo;
	                        break;
                        case 'q':
                            pValue ? pr.keyword = pValue : null;
                            searchInfo += "&q="+api.param.q;	
                            break;
                        case 'fromDt':
                            pValue ? pr.date.from = pValue : null;
                        	searchInfo += "&startdate="+api.param.fromDt;
                            break;
                        case 'toDt':
                            pValue ? pr.date.to = pValue : null;
                        	searchInfo += "&enddate="+api.param.toDt;
                            break;
                        case 'display':
                            pValue ? pr.page.count = pValue : null;
                            break;
                        case 'sort':
                        	...
                        	searchInfo += "&sort="+api.param.sort;
                            break;
                        case 'nstCd':
                            if (api.nstList) {
                                var nstCheck = $.grep(openAPI.nstList, function(e) {
            ...
            /**
             * http://opengov.seoul.go.kr/openapi?gvrncd=100019&display=10&pageno=1;
             * 여기서 불러옴
             */
            $.ajax({
                type: 'GET',
                url: 'https://opengov.seoul.go.kr/openapi'+searchInfo,
                //url: 'http://opengov.seoul.go.kr/openapi?gvrncd=100019&display=10&pageno=1&q=코로나&enddate=2021/05/05&sort=date',
                contentType: 'application/json; charset=UTF-8',
                dataType: 'JSONP',
                success: function(data) {
                	var keyList = ['astmtname','brm_cl_all_nm','deptcd','deptname','dlsrname','docnum','gvrncd','gvrnname','gvrnrgstnum','kwrd','makedate','og_url','prodman','regdate','score','title','url','view_count'];
                	var rtnList = [];
                	///
                	if(data.item){
                		for (var i in data.item){
                			var rtnData = {};
                			var idxKey = data.item[i].docnum;
                			
                			for (var k in keyList) {
                                rtnData[keyList[k]] = data.item[i][keyList[k]]
                            }
            	           rtnData['PRDNDT'] = data.item[i].regdate;
                           rtnData['PRDNNSTRGSTNO'] = data.item[i].docnum;
                           rtnList.push(rtnData);
                		};
                		callback({
                			data: rtnList,
                			info: {
                				pageNo: api.param.pageNo ,

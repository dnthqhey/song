...
<%@ page import="kr.co.hanshinit.NeoCMS.cop.pum.pui.service.PopupItemService" %>
<%@ page import="kr.co.hanshinit.NeoCMS.cop.pum.pui.service.PopupItemVO" %>
<%@ page import="kr.co.hanshinit.NeoCMS.cmm.service.SearchCriteria" %>
<%
	...
	<c:forEach items="${popupItemList}" var="popupItem">
		<c:choose>
			<c:when test="${popupItem.type eq 'window'}">
				<script>
				//<![CDATA[
					if(cookie.get("popup${sNumber}") != "done") {
				...
				//<![CDATA[
					function closePopup${sNumber}() {
						if($("#today_close${sNumber}").is(":checked")) {
							cookie.set("popup${sNumber}", "done" , 1);
						}
